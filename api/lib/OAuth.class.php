...
class OAuth {
    ...
    public function authenticate(){
        if($this->access_token != null){
            $query = "SELECT * FROM `$this->db_name`.`session` WHERE access_token = '$this->access_token';";
    ...
    public function newSession($valid_for = 7200, $reference_token = 'auth_grant'){
        ...
        $query = "INSERT INTO `$this->db_name`.`session` (`username`, `access_token`, `refresh_token`, `valid_for`, `reference_token`) 
    ...
    public function refreshAccess(){
        if($this->refresh_token != NULL and !$this->startsWith($this->refresh_token, 'd.')){
            $query = "SELECT * FROM `$this->db_name`.`session` WHERE refresh_token = '$this->refresh_token';";
            $result = mysqli_query($this->db, $query);
            if($result){
                $data = mysqli_fetch_assoc($result);
